...
contains
  ...
  subroutine evolve_field_model_b(psi, mu, P_field, cfg)
    real, intent(inout) :: psi(:,:)
    real, intent(in)    :: mu(:,:)
    ...
    do j = 1, cfg%Ly
      ...
      do i = 1, cfg%Lx
        ...
        psi(i,j) = psi(i,j) + cfg%dt * cfg%M * lap_mu - cfg%B*(psi(i,j)-cfg%psimean)* cfg%dt * (1-P_field(i,j))
        ! Pfield = 1 inside particle, 0 outside
      enddo
    enddo
