program main
  ...
  integer, allocatable, dimension(:,:) :: P_field
  ...
  allocate( P_field(cfg%Lx, cfg%Ly) )
  ...
  elseif (equilibrated_found) then 
      ...
      if (cfg%custom_init) then
        ...
        call initialize_custom_system_circle(particles, psi, cfg)          
      else
  ...
  call coupling(mu_total, psi, particles, P_field, cfg, curr_energy%coupling)
  ...
  do t = start_t, cfg%total_steps
    ...
    if ( cfg%sigma>0.0 ) call coupling(mu_total, psi, particles, P_field, cfg, curr_energy%coupling)

    ! B. Field Kinetics: Diffusion Step (Model B)
    ! d_psi/dt = M * Laplacian(mu_total)
    call evolve_field_model_b(psi, mu_total, P_field, cfg)
