...
contains
  ...
  subroutine load_parameters(cfg)
    ...
    ! Model B Parameters
    read(10, *) cfg%M, cfg%kappa, cfg%tau, cfg%u, cfg%B
    read(10, *) cfg%psimean 
  ...
  ! initialise custom psi as flat interface
  subroutine initialize_custom_system(particles, psi, cfg)
    ...
    do i = 1, cfg%Lx
      do j = 1, cfg%Ly
        ! iniialise as sinusoidal
        !psi(i,j) = sin(  2*pi * real(i) / real( cfg%Lx ))

        ! initalise as alternate
        if (i<cfg%Lx/2) then 
                psi(i,j) = sqrt( cfg%tau / cfg%u )
        else
                psi(i,j) = -sqrt( cfg%tau / cfg%u )
        endif
      enddo
...
! initialise custom psi as flat interface
  subroutine initialize_custom_system_circle(particles, psi, cfg)
    type(Particle_t), intent(inout) :: particles(:)
    real,             intent(inout) :: psi(:,:)
    type(Config_t),   intent(in)    :: cfg
    integer :: i, j, attempts, max_attempts
    ...
    do i = 1, cfg%Lx
      do j = 1, cfg%Ly
        ...
        r2 = (i - 0.5 * cfg%Lx) **2 + (j - 0.5 * cfg%Ly) **2
    ...
    do i = 1, cfg%Np
      ...
      do
        ...
        call random_number(r_rand); r_rand = 0.9 * r_rand * R_init

        tx = 0.5 * cfg%Lx + r_rand *cos( theta )
        ty = 0.5 * cfg%Ly + r_rand *sin( theta )
  ...
  end subroutine initialize_custom_system_circle
...
subroutine write_stats(t, psi, particles, cfg, energy)
    ...
    logical :: file_exists
    real :: centre_mass_x,centre_mass_y
    ...
    ! 
    call domain_masscentre(psi, cfg, centre_mass_x,centre_mass_y )
    ...
    write(41, '(I10, 5ES20.8E2)') t, domain_size, psiavg, psiabsavg, centre_mass_x, centre_mass_y 
    flush(41)
